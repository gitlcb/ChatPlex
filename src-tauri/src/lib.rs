            ...
            if line.starts_with("data: ") {
                ...
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(data) {
                    if let Some(delta) = parsed["choices"][0]["delta"].as_object() {
                        // Handle reasoning_content (thinking) separately from content
                        if let Some(reasoning) = delta.get("reasoning_content").and_then(|v| v.as_str()) {
                            if !reasoning.is_empty() {
                                eprintln!("[chat_stream] Reasoning: {:?}", &reasoning[..reasoning.len().min(50)]);
                        ...
                        if let Some(text) = delta.get("content").and_then(|v| v.as_str()) {
                            if !text.is_empty() {
                                eprintln!("[chat_stream] Content: {:?}", &text[..text.len().min(50)]);
                                let _ = app.emit("chat-stream-chunk", StreamChunk {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![chat_stream, eval_js])
        .setup(|_app| {
            // App is set up, webview management is done from the frontend
            Ok(())
        })
